...
_Instant rejections. These depend on TypeScript syntax (which core ESLint shouldn't own) or are implementation tactics rather than issue candidates._

- **DROP:** `func-style` ignore TS overload signatures inside `declare namespace` blocks.
- **DROP:** `func-style` handle overload + implementation around `export default function`.
- **DROP:** `no-duplicate-imports` handle `import type` + `export type ... from` parity.
- **DROP:** `no-empty-function` verify TS empty-method allowances for abstract/decorated overrides.
- **DROP:** Enforce option type parity when schema supports both array and object forms. _(Implementation tactic)_
- **DROP:** Add TS parser parity tests for rules with TS-adjacent options. _(Implementation tactic)_
