...
function messageTemplate(it) {
	...
	let encodedRuleUrl = "";
	if (ruleUrl) {
		const urlString = String(ruleUrl);
		// Block dangerous protocols to prevent XSS
		if (!urlString.toLowerCase().startsWith("javascript:") && // eslint-disable-line no-script-url -- allow javascript: check
			!urlString.toLowerCase().startsWith("vbscript:") &&
			!urlString.toLowerCase().startsWith("data:")) {
			encodedRuleUrl = encodeHTML(urlString);
		}
	}
...
<tr style="display: none;" class="f-${parentIndex}">
    ...
    <td>
        <a href="${encodedRuleUrl}" target="_blank" rel="noopener noreferrer">${ruleId ? encodeHTML(ruleId) : ""}</a>
    </td>
</tr>
`.trimStart();
}
