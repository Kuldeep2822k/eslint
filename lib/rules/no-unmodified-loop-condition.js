...
/**
 * Checks whether or not a given loop condition is modified by a given modifier.
 * @param {LoopConditionInfo} condition A loop condition info to check.
 * @param {Reference} modifier A reference to check.
 * @returns {boolean} `true` if the loop condition is modified by the modifier.
 */
function hasModifier(condition, modifier) {
...
function updateModifiedFlag(conditions, modifiers) {
	for (let i = 0; i < conditions.length; ++i) {
		...
		for (let j = 0; !condition.modified && j < modifiers.length; ++j) {
			 ...
			 * in the loop.
			 */
			const inLoop = hasModifier(condition, modifier);
