...
class ESLint {
	...
	async loadFormatter(name = "stylish") {
		 ...
		 * scoped npm packages also have slashes, so if it looks like a scoped package, it's only a file if it exists locally
		 */
		if (
			(!namespace && normalizedFormatName.includes("/")) ||
			(namespace && existsSync(path.resolve(cwd, normalizedFormatName)))
		) {
